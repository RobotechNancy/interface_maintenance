...
<div class="accordion-item bg-dark text-white-50">
    <h2 class="accordion-header" id="log_<?= $log->id ?>">
        <button class="accordion-button btn-dark bg-dark text-white-50" type="button" data-bs-toggle="collapse"
            data-bs-target="#collapse_log_<?= $log->id ?>" aria-expanded="false" aria-controls="log_<?= $log->id ?>">
    ...
    @if (json_last_error() == JSON_ERROR_NONE)
        ...
        <div id="collapse_log_<?= $log->id ?>" class="accordion-collapse collapse" aria-labelledby="log_<?= $log->id ?>"
            data-bs-parent="#accordionConsole">
                                ...
                                <ul class="list-group list-group-flush pt-2">
                                    ...
                                    @if (json_last_error() == JSON_ERROR_NONE)
                                        ...
                                        if (!empty($trame_can_env) && !empty($trame_can_rec)) {
                                            ...
                                            if ($test_addr == 'success') {
                                                $msg_addr = "Le destinataire du message envoyé est bien l'expéditeur du message reçu";
                                            } elseif ($test_addr == 'warning') {
                                                $msg_addr = "Le destinataire du message envoyé est différent de l'expéditeur du message reçu";
                                            } else {
                                                $msg_addr = "Le destinataire du message envoyé et/ou l'expéditeur du message reçu n'est pas spécifié dans la trame";
                                            }
                                            ...
                                            if ($test_emetteur == 'success') {
                                                $msg_emetteur = "L'expéditeur du message envoyé est bien le destinataire du message reçu";
                                            } elseif ($test_emetteur == 'warning') {
                                                $msg_emetteur = "L'expéditeur du message envoyé est différent du destinataire du message reçu";
                                            } else {
                                                $msg_emetteur = "L'expéditeur du message envoyé et/ou le destinataire du message reçu n'est pas spécifié dans la trame";
                                            }

                                            $test_code_fct = $trame_can_env->{"code_fct"} == $trame_can_rec->{"code_fct"} ? 'success' : 'warning';

                                            if ($test_code_fct == 'success') {
                                                $msg_code_fct = 'Les codes fonctions envoyés et reçus sont connus et cohérents';
                                            } else {
                                                $msg_code_fct = 'Les codes fonctions envoyés et reçus ne sont pas les mêmes';
                                            }

                                            $test_id_rep = $trame_can_env->{"id_rep"} == $trame_can_rec->{"id_rep"} ? 'success' : 'warning';

                                            if ($test_id_rep == 'success') {
                                                $msg_id_rep = 'Le numéro de trame de reçu est le même que celui envoyé';
                                            } else {
                                                $msg_id_rep = 'Le numéro de trame de reçu ne correspond pas à celui envoyé';
                                            }
                                        }
                                        ...
                                        if (!isset($test_emetteur)) {
                                            $test_emetteur = 'success';
                                        }
                                        if (!isset($test_code_fct)) {
                                            $test_code_fct = 'success';
                                        }
                                        if (!isset($test_id_rep)) {
                                            $test_id_rep = 'success';
                                        }
                                        ...
                                        if (!empty($trame_can_env)) {
                                            ...
                                            if ($test_is_rep_env == 'success') {
                                                $msg_is_rep_env = "Le type de trame spécifié à l'envoi est connu et cohérent";
                                            } elseif ($test_is_rep_env == 'warning') {
                                                $msg_is_rep_env = "Le type de trame spécifié à l'envoi est connu mais incohérent";
                                            } else {
                                                $msg_is_rep_env = "Aucun type de trame n'a été spécifié à l'envoi";
                                            }
                                        ...
                                        if (!empty($trame_can_rec)) {
                                            ...
                                            if ($test_is_rep_rec == 'success') {
                                                $msg_is_rep_rec = 'Le type de trame spécifié à la réception est connu et cohérent';
                                            } elseif ($test_is_rep_rec == 'warning') {
                                                $msg_is_rep_rec = 'Le type de trame spécifié à la réception est connu mais incohérent';
                                            } else {
                                                $msg_is_rep_rec = "Aucun type de trame n'a été spécifié à la réception";
                                            }
                                                            ...
                                                            <span class="fw-bold" style="cursor: pointer;"
                                                                ...
                                                                @isset($test_trame_can_env)
                                                                    @isset($msg_trame_can_env)
                                                                        <span class="badge text-bg-{{ $test_trame_can_env }}"
                                                                            data-bs-toggle="tooltip" data-bs-placement="right"
                                                                            data-bs-title="{{ $msg_trame_can_env }}"><i
                                                                                class='fa-solid {{ $test_icon[$test_trame_can_env] }}'></i></span>
                                                                    @endisset
                                                                @endisset
                                                                <span class="text-muted fst-italic fw-normal"
                                                                    ...
                                                                    <div class="ms-2 me-auto">
                                                                        ...
                                                                        <span class="text-white-50">
                                                                            ...
                                                                            @isset($test_addr)
                                                                                @isset($msg_addr)
                                                                                    <span
                                                                                        class="badge text-bg-{{ $test_addr }}"
                                                                                        data-bs-toggle="tooltip"
                                                                                        data-bs-placement="right"
                                                                                        data-bs-title="{{ $msg_addr }}"><i
                                                                                            class='fa-solid {{ $test_icon[$test_addr] }}'></i></span>
                                                                                @endisset
                                                                            @endisset
                                                                        </span>
                                                                    </div>
                                                                    ...
                                                                    <div class="ms-2 me-auto">
                                                                        ...
                                                                        <span class="text-white-50">
                                                                            ...
                                                                            @isset($test_emetteur)
                                                                                @isset($msg_emetteur)
                                                                                    <span
                                                                                        class="badge text-bg-{{ $test_emetteur }}"
                                                                                        data-bs-toggle="tooltip"
                                                                                        data-bs-placement="right"
                                                                                        data-bs-title="{{ $msg_emetteur }}"><i
                                                                                            class='fa-solid {{ $test_icon[$test_emetteur] }}'></i></span>
                                                                                @endisset
                                                                            @endisset
                                                                        </span>
                                                                    </div>
                                                                    ...
                                                                    <div class="ms-2 me-auto">
                                                                        ...
                                                                        <span class="text-white-50">
                                                                            ...
                                                                            @isset($test_code_fct)
                                                                                @isset($msg_code_fct)
                                                                                    <span
                                                                                        class="badge text-bg-{{ $test_code_fct }}"
                                                                                        data-bs-toggle="tooltip"
                                                                                        data-bs-placement="right"
                                                                                        data-bs-title="{{ $msg_code_fct }}"><i
                                                                                            class='fa-solid {{ $test_icon[$test_code_fct] }}'></i></span>
                                                                                @endisset
                                                                            @endisset
                                                                        </span>
                                                                    </div>
                                                                    ...
                                                                    <div class="ms-2 me-auto">
                                                                        <div class="">ID trame
                                                                            ...
                                                                            @isset($test_id_rep)
                                                                                @isset($msg_id_rep)
                                                                                    <span
                                                                                        class="badge text-bg-{{ $test_id_rep }}"
                                                                                        data-bs-toggle="tooltip"
                                                                                        data-bs-placement="right"
                                                                                        data-bs-title="{{ $msg_id_rep }}"><i
                                                                                            class='fa-solid {{ $test_icon[$test_id_rep] }}'></i></span>
                                                                                @endisset
                                                                            @endisset
                                                                        </div>
                                                                    </div>
                                                                        ...
                                                                        <span class="text-white-50">
                                                                            ...
                                                                            @isset($test_is_rep_env)
                                                                                @isset($msg_is_rep_env)
                                                                                    <span
                                                                                        class="badge text-bg-{{ $test_is_rep_env }}"
                                                                                        data-bs-toggle="tooltip"
                                                                                        data-bs-placement="right"
                                                                                        data-bs-title="{{ $msg_is_rep_env }}"><i
                                                                                            class='fa-solid {{ $test_icon[$test_is_rep_env] }}'></i></span>
                                                                                @endisset
                                                                            @endisset
                                                                        </span>
                                                                    </div>
                                                            ...
                                                            <span class="fw-bold" style="cursor: pointer;"
                                                                ...
                                                                @isset($test_trame_can_rec)
                                                                    @isset($msg_trame_can_rec)
                                                                        <span class="badge text-bg-{{ $test_trame_can_rec }}"
                                                                            data-bs-toggle="tooltip" data-bs-placement="right"
                                                                            data-bs-title="{{ $msg_trame_can_rec }}"><i
                                                                                class='fa-solid {{ $test_icon[$test_trame_can_rec] }}'></i></span>
                                                                    @endisset
                                                                @endisset
                                                                <span class="text-muted fst-italic fw-normal"
                                                                            ...
                                                                            @isset($test_code_fct)
                                                                                @isset($msg_code_fct)
                                                                                    <span
                                                                                        ...
                                                                                        data-bs-toggle="tooltip"
                                                                                        data-bs-placement="right"
                                                                                        @isset($msg_code_fct) data-bs-title="{{ $msg_code_fct }}" @endisset><i
                                                                                            class='fa-solid {{ $test_icon[$test_code_fct] }}'></i></span>
                                                                                @endisset
                                                                            @endisset
                                                                        </span>
                                                                        ...
                                                                        <span class="text-white-50">
                                                                            ...
                                                                            @isset($test_is_rep_rec)
                                                                                @isset($msg_is_rep_rec)
                                                                                    <span
                                                                                        class="badge text-bg-{{ $test_is_rep_rec }}"
                                                                                        data-bs-toggle="tooltip"
                                                                                        data-bs-placement="right"
                                                                                        data-bs-title="{{ $msg_is_rep_rec }}"><i
                                                                                            class='fa-solid {{ $test_icon[$test_is_rep_rec] }}'></i></span>
                                                                                @endisset
                                                                            @endisset
                                                                        </span>
                                                                    </div>
                                                ...
                                                <div class="card text-bg-dark h-100" style="border:none;">
                                                    <div class="card-body">
                                                        ...
                                                        <span class="fw-bold" style="cursor: pointer;"
                                                            ...
                                                            </span><span class="text-muted fst-italic fw-normal"
                                        ...
                                        @if (count($errors_json) > 0)
                                            <hr>
                                            <div class="mb-3">
                                                <p class="ps-3 text-muted"><i
                                                        class="fa-solid fa-triangle-exclamation"></i> Des erreurs ont
                                                    été détectées lors de la lecture du log : </p>
                                                <ul>
