<x-app-layout>
    ...
    <div class="row">
        ...
        <div class="col ms-lg-4">
            ...
            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-2 g-4 p-3">
                        ...
                        <div aria-hidden="true" class="card text-bg-dark border border-light border-opacity-25">
                                ...
                                <h5 class="card-title">
                                    <span>{{ $user->name }}
                                        ...
                                        @if (Cache::has('user-is-online-' . $user->email))
                                            <span class="text-success fs-6 ms-2 fst-italic"><small>En
                                                    ligne</small></span>
                                        @else
                                            <span
                                                class="text-danger fs-6 ms-2 fst-italic"><small>Déconnecté</small></span>
                                        @endif


                                        @if (Auth::user()->id == $user->id)
                                            <span class="badge rounded-pill ms-2 text-bg-info"><small>Mon
                                                    compte</small></span>
                                        @endif
                                    </span>
                                </h5>
                            ...
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item text-bg-dark border border-white border-opacity-10"><i
                                        class="fa-solid fa-id-card-clip pe-1"></i> Identifiant : {{ $user->id }}
                                </li>
                                <li class="list-group-item text-bg-dark border border-white border-opacity-25"><i
                                        class="fa-solid fa-at pe-1"></i> @lang('Email') : <a
                                        href="mailto:{{ $user->email }}" class="text-white"
                                        style="text-decoration: none;">{{ $user->email }}</a>
                                </li>
                                <li class="list-group-item text-bg-dark border border-white border-opacity-10"><i
                                        class="fa-solid fa-ranking-star pe-1"></i> @lang('Role') :
                                    {{ $role }}</li>
                            </ul>
                            ...
                            @if (Auth::user()->role == 2 || $user->id == Auth::user()->id)
                                <div class="card-footer text-center m-2">
                                    <a class="btn btn-sm btn-primary" href="{{ route('edit', ['id' => $user->id]) }}"><i
                                            class="fa-solid fa-pen-to-square"></i> Éditer <span
                                    ...
                                    <form id="delete{{ $user->id }}"
                                        action="{{ route('delete', ['user' => $user]) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                    </form>
