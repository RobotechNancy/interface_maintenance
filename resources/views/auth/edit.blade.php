<x-app-layout>
    ...
    <div class="row">
        ...
        <div class="col ms-lg-4">
            ...
            <p class="ps-3 text-muted"><small><i class="fa-solid fa-circle-info"></i> Dernière modification de ce profil
                    le {{ $user->updated_at->format('d/m/Y') }} à
            ...
            <form method="POST" action="{{ route('update', ['user' => $user]) }}" class="p-3">
                        ...
                        <div class="input-group mb-4">
                            ...
                            <div class="form-floating <?php if (!empty($errors->get('name'))) {
                                ...
                                <input class="form-control bg-dark text-white <?php if (!empty($errors->get('name'))) {
                                    ...
                                    id="name" name="name" value="{{ $user->name }}" placeholder="Alex"
                                    aria-describedby="user" autofocus>
                ...
                <div class="mb-4">
                    <div class="form-floating">
                        <select class="form-select bg-dark text-white <?php if (!empty($errors->get('role'))) {
                            ...
                            <option value=2 @if ($user->role == 2) selected @endif disabled>Administrateur
                            </option>
                        </select>
                    ...
                    @if (Auth::user()->role != 2 && Auth::user()->id != $user->id)
                        <p class="mt-2 text-warning"><small><i class="fa-solid fa-triangle-exclamation"></i> Il n'est
                                pas possible de modifier le rôle de l'utilisateur car
                                vous n'êtes pas administrateur.</small></p>
                    @elseif (Auth::user()->role != 2 && Auth::user()->id == $user->id)
                        <p class="mt-2 text-warning"><small><i class="fa-solid fa-triangle-exclamation"></i> Il n'est
                                pas possible de modifier le rôle de votre compte car vous
                                n'êtes pas administrateur.</small></p>
                    @elseif ($user->role == 2 && Auth::user()->id != $user->id)
                        <p class="mt-2 text-warning"><small><i class="fa-solid fa-triangle-exclamation"></i> Il n'est
                                pas possible de modifier le rôle de l'utilisateur car il
                ...
                <div class="btn-group" role="group">
                    <button type="submit" class="btn btn-primary">Enregistrer les modifications <i
            ...
            <a role="button" class="btn btn-danger ms-3" onclick="event.preventDefault(); $('#form_delete').submit();">
                Supprimer l'utilisateur <i class="fa-solid fa-trash-can"></i>
            </a>

            @if (session()->has('success'))
                <x-notification title="Modification du profil" color="text-bg-success">{{ session('success') }}
                </x-notification>
            @endif
            @if (session()->has('warning'))
                <x-notification title="Modification du profil" color="text-bg-warning">{{ session('warning') }}
                </x-notification>
            @endif
        </div>
    </div>
